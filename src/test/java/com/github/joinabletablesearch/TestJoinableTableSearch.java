package com.github.joinabletablesearch;

import com.github.joinabletablesearch.deserializer.TableDeserializer;
...
public class TestJoinableTableSearch {
    ...
    public void testSearchLargeTable() {
        ...
        try {
            Table table = this.getTable("largeTable.json");
            long start = System.currentTimeMillis();
    ...
    public void testSearchMultiTable() {
        ...
        int[] tables = {4, 12, 25, 50, 100, 200, 401, 802, 1599, 3143, 6102};
        //int[] tables = {4,12,25,50,100};
        for (int n : tables) {
            try {
                Table table = this.getTable("tables/table-" + n + ".json");
                long start = System.currentTimeMillis();
                System.out.println("Start searching...");
                List<Integer> topKSet = searcher.search(table.getColumns().get(0), 2, table.getMongoId().getOid());
                long time = (System.currentTimeMillis() - start) / 1000;
